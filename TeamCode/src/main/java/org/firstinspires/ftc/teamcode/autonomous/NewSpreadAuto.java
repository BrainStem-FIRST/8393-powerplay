...
public class NewSpreadAuto extends LinearOpMode {
    ...
    public NewSpreadAuto(AutoOrientation side) {
        this.side = side;
        switch (side) {
            case RIGHT:
                LEFTSIDE = false;
                initialApproach = new Pose2d(initialApproach.getX() , -initialApproach.getY());
                initialTangent = 80;
    ...
    private TrajectorySequence initializeTrajectories(AutoBrainSTEMRobot robot, SampleMecanumDrive drive) {
        ...
        TrajectorySequence deliverPreload = drive.trajectorySequenceBuilder(startPosition)
                ...
                .UNSTABLE_addTemporalMarkerOffset(0.1, () -> {
                    telemetry.addData("Auto Position :", "0.1 Offset");
                    telemetry.update();
